...
try:
    ...
    @main.group()
    def staging():
        """Manage local staging area for secure issue preparation."""
        pass
    
    @staging.command()
    def list():
        """List all staged issues."""
        try:
            from jira_tools.staging import local_staging
            from rich.table import Table
            
            staged_files = local_staging.list_staged()
            
            if not staged_files:
                console.print("No staged issues found")
                console.print("💡 Create an epic with --dry-run to stage it locally")
                return
            
            table = Table(title="Staged Issues")
            table.add_column("File", style="cyan")
            table.add_column("Type", style="green")
            table.add_column("Summary", style="yellow", max_width=40)
            table.add_column("Template", style="blue")
            table.add_column("Staged", style="magenta")
            table.add_column("Size", style="red")
            
            for file_info in staged_files:
                size_kb = file_info['file_size'] / 1024
                staged_time = file_info['modified'].strftime('%m-%d %H:%M')
                ...
                table.add_row(
                    file_info['filename'],
                    file_info['issue_type'],
                    summary,
                    file_info['template_used'],
                    staged_time,
                    f"{size_kb:.1f}KB"
                )
            
            console.print(table)
            console.print(f"\n📁 Staging directory: {local_staging.staging_dir}")
            
        except Exception as e:
            console.print(f"❌ Error listing staged files: {e}")
    
    @staging.command()
    @click.argument('filename')
    def view(filename):
        """View the contents of a staged issue."""
        try:
            from jira_tools.staging import local_staging
            
            # Add .jira-staged extension if not provided
            if not filename.endswith('.jira-staged'):
                filename += '.jira-staged'
            
            payload = local_staging.load_staged_file(filename)
            
            if not payload:
                console.print(f"❌ No payload found in {filename}")
                return
            
            # Display formatted JSON
            console.print(f"📄 Viewing staged file: [yellow]{filename}[/yellow]")
            console.print("=" * 60)
            ...
            console.print(f"❌ Error viewing staged file: {e}")
    
    @staging.command()
    @click.argument('filename')
    def remove(filename):
        """Remove a staged issue file."""
        try:
            from jira_tools.staging import local_staging
            from rich.prompt import Confirm
            ...
            if not Confirm.ask(f"Remove staged file '{filename}'?"):
                console.print("Remove cancelled")
                return
            
            success = local_staging.remove_staged_file(filename)
            if success:
            ...
            console.print(f"❌ Error removing staged file: {e}")
    
    @staging.command()
    @click.option('--days', default=7, help='Remove files older than N days')
    @click.option('--force', is_flag=True, help='Skip confirmation prompt')
    def clean(days, force):
        ...
        try:
            ...
            if not force:
                if not Confirm.ask(f"Remove staged files older than {days} days?"):
                    console.print("Clean cancelled")
                    return
            
            removed_count = local_staging.clean_old_files(days)
            
            if removed_count > 0:
                console.print(f"✅ Removed {removed_count} old staged files")
            else:
                console.print("No old files to remove")
                
        except Exception as e:
            console.print(f"❌ Error cleaning staged files: {e}")
    
    @staging.command()
    def audit():
        """Perform security audit of staging directory."""
        try:
            from jira_tools.staging import local_staging
            from rich.panel import Panel
            
            results = local_staging.validate_staging_security()
            ...
            console.print(Panel(
                f"[{status_color}]{status_icon} Staging Security: {status_text}[/{status_color}]",
                title="Staging Security Audit"
            ))
            ...
            if results['secure'] and not results['warnings'] and not results['recommendations']:
                console.print("\n✅ Staging directory is properly secured!")
                
        except Exception as e:
            console.print(f"❌ Error auditing staging security: {e}")
    
    @staging.command()
    def status():
        """Show staging directory status and statistics."""
        try:
            from jira_tools.staging import local_staging
            from rich.table import Table
            
            console.print("📂 Local Staging Status")
            console.print(f"Directory: {local_staging.staging_dir}")
            
            # Check if directory exists
            if not local_staging.staging_dir.exists():
                console.print("❌ Staging directory does not exist")
                return
            ...
            if not staged_files:
                console.print("✅ No staged files (clean staging area)")
                return
            
            # Statistics table
            stats_table = Table(title="Staging Statistics")
            ...
            for file_info in staged_files:
                issue_type = file_info['issue_type']
                template = file_info['template_used']
                
                type_counts[issue_type] = type_counts.get(issue_type, 0) + 1
                template_counts[template] = template_counts.get(template, 0) + 1
                total_size += file_info['file_size']
            
            stats_table.add_row("Total Files", str(len(staged_files)))
            stats_table.add_row("Total Size", f"{total_size / 1024:.1f} KB")
            
            # Issue type breakdown
            for issue_type, count in sorted(type_counts.items()):
                stats_table.add_row(f"{issue_type.title()} Issues", str(count))
            
            console.print(stats_table)
            
            # Security status
            security_results = local_staging.validate_staging_security()
            security_color = "green" if security_results['secure'] else "red"
            security_text = "✅ Secure" if security_results['secure'] else "⚠️  Issues Found"
            
            console.print(f"\nSecurity Status: [{security_color}]{security_text}[/{security_color}]")
            
        except Exception as e:
            console.print(f"❌ Error checking staging status: {e}")
